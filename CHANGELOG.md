...
- **Composer context meter and model switcher.** The chat input shows
  ...
  the name only.
- **Per-model max output on the Models page.** Each catalog row now
  has a max-output field next to the context window (official
  prefills for GPT / Zhipu / Kimi where the vendor publishes one;
  otherwise 8192). Saving it sets that model's completion budget
  (`max_tokens` / `max_completion_tokens`); compaction reserves the
  same number. Unset rows keep the 8192 agent default — official
  caps are form defaults only, not a silent runtime change.
