...
func TestIsTransientLLMError(t *testing.T) {
	...
	if isTransientLLMError(errors.New("API error 402: Insufficient Balance")) {
		t.Fatal("402 must not be retried")
	}
...
func TestLLMRetryRetriesTransientThenSucceeds(t *testing.T) {
	// llmRetry sleeps 1s before the second attempt. Keep this test
	// honest but short by using a real transient error once.
	calls := 0
	start := time.Now()
