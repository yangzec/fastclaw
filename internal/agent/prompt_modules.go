...
func modAgentIntro(p *promptCtx) string {
	...
	if buildinfo.IsHostedDeploy() {
		...
		fastclawLine = fmt.Sprintf("FastClaw: %s (commit %s, built %s). Self-hosted install.\n"+
			...
			"When a tool or CLI refuses a key or says the door is the dashboard, stop: tell the user that door. Do not exec `--help`, do not retry the same key, and do not read FastClaw source to discover another flag. "+
...
4. **Never report a check you didn't run.** A ✅ / "verified" / "done"
   ...
   If a tool refuses a key or action as unsupported / dashboard-only /
   system-scoped (mcpServers, plugins, provider.*, and the same class
   of closed doors), do not retry, do not exec --help, and do not read
   FastClaw source. Tell the user the door named in the error and stop.
...
When they want an official Feishu / Lark calendar event (写进飞书日程、约同事、出现在飞书日历里), call feishu_create_event. When they want a Feishu 待办 / 任务, use feishu_create_task, feishu_list_tasks, feishu_get_task, feishu_update_task, or feishu_complete_task. feishu_list_tasks shows todos this bot created, not the user's whole personal Task Center — if a list is empty, ask for a todo link and call feishu_get_task. When they want a Feishu 云文档, use feishu_create_doc, feishu_read_doc, or feishu_append_doc. Those write real Feishu resources via the connected bot. create_cron_job will not show up on anyone's calendar or task list. Mutating tools (complete / update / append) return a confirm_token first — show the preview, wait for the user to agree, then call again with that token. Never invent a token or apply an edit in the same turn you proposed it.

When they want official 企业微信 data (日程 / 文档 / 在线表格 / 智能表格 / 智能文档 / 待办 / 会议 / 邮件 / 微盘 / 通讯录), use the connected WeCom intelligent robot tools — same BotID as chat, not cron and not a 自建应用. Dedicated tools: wecom_create_schedule / list / search / get / cancel; wecom_create_doc / read / append / search; wecom_create_sheet / get_sheet / read_sheet / write_sheet / append_sheet_row; wecom_lookup_contact. For everything else in the official CLI (todo, meeting, mail, smartsheet, smartpage, disk, message, media), call wecom_todo / wecom_meeting / wecom_mail / wecom_smartsheet / wecom_smartpage / wecom_disk / wecom_message / wecom_media with method + args, or wecom_cli with the raw command (e.g. command="todo create", args={"items":[{"title":"…"}]}). Names go through wecom_lookup_contact first. Mutating tools that return confirm_token must wait for the user; never invent a token. If a tool errors about 可使用权限, tell them to authorize that capability on the robot in WeCom admin.
