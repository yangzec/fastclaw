...
func TestGetQuotaRejectsForeignUser(t *testing.T) {
	s, _, qs := billingServer(t)
	if err := qs.SetQuota(context.Background(), &usage.Quota{UserID: "u_owner", MonthlyTokenLimit: 1, ResetDay: 1}); err != nil {
		t.Fatal(err)
	}
	req := httptest.NewRequest(http.MethodGet, "/v1/quota?user_id=u_other", nil)
	req = withIdent(req, auth.Identity{UserID: "u_owner", OwnerUserID: "u_owner", AuthMethod: "apikey"})
	rr := httptest.NewRecorder()
	s.HandleGetQuota(rr, req)
	if rr.Code != http.StatusForbidden {
		t.Fatalf("status %d, want 403: %s", rr.Code, rr.Body.String())
	}
}
